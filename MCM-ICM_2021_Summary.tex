...
\begin{center}
	...
	\end{center}
\begin{abstract}
As Asian Hornet invadeds Washington State, the public voluntarily provide information about the insects they have seen or captured which looked  like the hornet. And to analysis the spread of the hornet and distinguish  the insect, we use \textbf{hypothesis testing theory} to prove that the spread of hornet is random and the pest has been eradicated gradually in Washington States.  

The sighting reports contains images, videos, note texts, spatial-temporal information so it is a multimodal classification. To fully exploit the data set, we propose a \textbf{two-stage multimodal classifier}.

\begin{enumerate}[\bf 1.]
	
	
	\item In the first stage, we propose different methods to process multimodal inputs and transform them into scaled features.
	For videos, we convert them to images by  \textbf{key frames extractions} with FFmprobe and FFmpeg tools.

	For images whose backgrounds differ significantly, we crop them into sub-images containing potential hornet only and established an \textbf{Inception v4} network based on images in the reports.The network outputs predictions of class probability, which represents a feature of images. Croping the images require annotated object boxs in each picture, we annotate some samples manually and then train a \textbf{YOLOv5} object detection network to automate the process.

	For spatial-temporal information, we use inverse and exponential function to describe
the decay of hornet spread over space and time.
	
	For texts, we use \textbf{n-gram} and \textbf{ridge regression} to classify the report note and also output the class prediction probablity as a feature for the second stage.
	
	\item In the second stage, we combine all the features together to use \textbf{logistic regression} as the final classifier. Grid search with cross validation is introduced to tune the model.
...
\begin{thebibliography}{99}
	...
	\bibitem{website}Washington State Department of Agriculture. 2020 Asian Giant Hornet Public Dashboard. https://agr.wa.gov/departments/insects-pests-and-weeds/insects/hornets/data.
	...
	\bibitem{Yolo}Kuznetsova, Anna, et al. “Detecting Apples in Orchards Using YOLOv3 and YOLOv5 in General and Close-Up Images.” International Symposium on Neural Networks, 2020, pp. 233–243.
	\bibitem{inception}Szegedy, Christian, et al. "Inception-v4, inception-resnet and the impact of residual connections on learning." Proceedings of the AAAI Conference on Artificial Intelligence. Vol. 31. No. 1. 2017.
	\bibitem{focal}Lin, Tsung-Yi, et al. “Focal Loss for Dense Object Detection.” 2017 IEEE International Conference on Computer Vision (ICCV), 2017, pp. 2999–3007.
	\bibitem{Smote}Van der Maaten, Laurens, and Geoffrey Hinton. "Visualizing data using t-SNE." Journal of machine learning research 9.11 (2008).
	...
	\bibitem{gscv}Pedregosa, Fabian, et al. “Scikit-Learn: Machine Learning in Python.” Journal of Machine Learning Research, vol. 12, no. 85, 2011, pp. 2825–2830.
\end{thebibliography}
